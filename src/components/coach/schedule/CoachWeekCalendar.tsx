                        ...
                        <TouchableOpacity
                          ...
                          style={[
                            styles.scheduleBlock,
                            {
                              ...
                              opacity: item.kind === 'open' && !item.publiclyBookable ? 0.58 : 1,
                            },
                          ]}
...
function SlotPopover({
  ...
  const showBelow = anchor.y < window.height * 0.48;
  const top = showBelow
    ? Math.min(anchor.y + anchor.height + 12, window.height - 360)
    : Math.max(12, anchor.y - 330);
  const pointerLeft = Math.max(18, Math.min(anchor.x + anchor.width / 2 - left - 9, width - 28));
  ...
  return (
    <Modal transparent visible animationType="fade" onRequestClose={onClose}>
      <TouchableOpacity style={styles.popoverBackdrop} onPress={onClose} activeOpacity={1}>
        <TouchableOpacity
          activeOpacity={1}
          style={[styles.popover, { width, left, top }]}>
          <View style={[
            styles.pointer,
            showBelow ? styles.pointerTop : styles.pointerBottom,
            { left: pointerLeft },
          ]} />
          ...
          {item.kind === 'lesson' && (
            <>
              <Text style={styles.detail}>
                {item.request.lessonType.replaceAll('_', ' ')}
              ...
              <PopoverAction
                ...
                onPress={() => Alert.alert(
                  'Lesson Details',
                  `${item.request.playerName ?? 'Player'}\n${formatTime(item.start)} - ${formatTime(item.end)}\n${item.request.notes ?? 'No student notes.'}`,
                )}
              />
              <PopoverAction
                label="Cancel Lesson"
...
function useStyles(theme: ThemeTokens) {
  return useMemo(() => StyleSheet.create({
    ...
    pointerTop: { top: -10, borderLeftWidth: 1, borderTopWidth: 1 },
    pointerBottom: { bottom: -10, borderRightWidth: 1, borderBottomWidth: 1 },
