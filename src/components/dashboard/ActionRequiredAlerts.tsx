...
import { AlertCircle, Trophy, GraduationCap, Wrench, X, ChevronRight } from "lucide-react";
...
export const ActionRequiredAlerts = () => {
  ...
  useEffect(() => {
    if (currentUser) loadAlerts();
  }, [currentUser, isHOAMode]);
  ...
  const loadAlerts = async () => {
    ...
    try {
      ...
      // Tennis alerts hidden in HOA mode
      if (!isHOAMode) {
      ...
      // HOA maintenance alerts always shown
  ...
  return (
    <div className="space-y-2">
      {visibleAlerts.map(alert => (
        <Card key={alert.id} className={`p-3 border ${alert.color} flex items-center gap-3`}>
          ...
          <div className="flex items-center gap-2">
            <Button asChild variant="ghost" size="sm" className="h-8 px-2">
              <Link to={alert.link} className="flex items-center gap-1">View<ChevronRight className="h-3 w-3" /></Link>
            </Button>
            <Button variant="ghost" size="sm" className="h-8 w-8 p-0" onClick={() => dismissAlert(alert.id)}>
              <X className="h-3 w-3" />
            </Button>
          </div>
        </Card>
