...
import { Calendar, Clock, MapPin, Trophy, GraduationCap, CalendarCheck, Waves } from "lucide-react";
...
export const UpcomingActivitySnapshot = ({ forecast = {} }: UpcomingActivitySnapshotProps) => {
  ...
  useEffect(() => {
    if (currentUser) loadUpcomingActivity();
  }, [currentUser, bookings, isHOAMode]);
  ...
  const loadUpcomingActivity = async () => {
    ...
    try {
      ...
      // HOA amenity reservations always shown
      ...
      // Tennis activities only in tennis mode
      if (!isHOAMode) {
        ...
        const { data: matches } = await supabase
          .from("match_requests")
          .select("id, date, time_start, location, match_type, challenger:profiles!match_requests_challenger_id_fkey(full_name, avatar_url), opponent:profiles!match_requests_opponent_id_fkey(full_name, avatar_url)")
  ...
  if (loading) {
    return (
      <Card className="animate-pulse">
        ...
        <CardContent><div className="space-y-3">{[1, 2, 3].map(i => <div key={i} className="h-20 bg-muted rounded-xl"></div>)}</div></CardContent>
      </Card>
    );
  }
    ...
    <Card className="overflow-hidden">
      <CardHeader className="pb-3">
        <CardTitle className="text-lg flex items-center justify-between">
          <div className="flex items-center gap-2"><Calendar className="h-5 w-5 text-primary" />Coming Up</div>
          <Link to="/upcoming" className="text-sm font-medium text-primary hover:underline">View All</Link>
        </CardTitle>
      </CardHeader>
      <CardContent className="pt-0 space-y-3">
        {upcomingItems.length > 0 ? (
          upcomingItems.map((item, index) => {
            ...
            return (
              <div key={`${item.type}-${item.id}`}
                className={`rounded-xl border transition-all ${isFirst ? "bg-primary text-primary-foreground p-4 border-primary" : "bg-card p-3 border-border hover:border-primary/20"}`}>
                {isFirst && item.badge && (
                  <Badge variant="secondary" className="mb-2 bg-primary-foreground/20 text-primary-foreground text-xs">{item.badge}</Badge>
                )}
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    <div className={`font-semibold ${isFirst ? "text-lg" : "text-sm"} capitalize`}>{item.title}</div>
                    {item.subtitle && (
                      <div className={`text-sm ${isFirst ? "text-primary-foreground/80" : "text-muted-foreground"}`}>{item.subtitle}</div>
                    )}
                    <div className={`flex items-center gap-3 mt-2 text-xs ${isFirst ? "text-primary-foreground/80" : "text-muted-foreground"}`}>
                      <span className="flex items-center gap-1"><Calendar className="h-3 w-3" />{formatDate(item.date)}, {formatTime12Hour(item.time)}</span>
                    ...
                    <div className="flex -space-x-2">
                      {item.avatars.slice(0, 2).map((avatar, idx) => (
                        <Avatar key={idx} className="w-8 h-8 border-2 border-background">
                          ...
                          <AvatarFallback className="bg-muted text-xs">{idx === 0 ? "P1" : "P2"}</AvatarFallback>
                        </Avatar>
                      ))}
                    </div>
                  )}
                  {!isFirst && weatherData && (
                    <WeatherBadge temperature={weatherData.temperature} condition={weatherData.condition} compact />
                  )}
                </div>
              </div>
            );
          ...
          <div className="text-center py-8">
            ...
            <Button asChild size="sm"><Link to="/reserve-court">Book an amenity</Link></Button>
          </div>
        )}
      </CardContent>
    </Card>
