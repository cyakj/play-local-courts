...
const CondoManagerBottomNav = () => {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 bg-white border-t border-cm-border z-50 safe-area-pb">
          ...
          return (
            ...
            >
              <div className="relative">
                <Icon className={`h-5 w-5 transition-colors ${active ? 'text-cm-cyan' : 'text-cm-text-light'}`} />
                {item.badge && item.badge > 0 && (
                  <div className="absolute -top-1 -right-1.5 bg-cm-danger text-white rounded-full w-4 h-4 flex items-center justify-center text-[9px] font-extrabold">
                    {item.badge}
                  </div>
                )}
              </div>
              <span className={`text-[10px] font-bold transition-colors ${active ? 'text-cm-cyan' : 'text-cm-text-light'}`}>
                {item.label}
              </span>
              {active && <div className="w-5 h-[3px] rounded-full bg-cm-cyan" />}
            </Link>
          );
        })}
      </div>
