...
export type WeatherScore = 'Excellent' | 'Good' | 'Fair' | 'Poor';

export const WEATHER_SCORE_COLORS: Record<WeatherScore, string> = {
  Excellent: Colors.positive,   // #2FD98B
  Good:      Colors.cyan,       // #2DE0FF
  Fair:      Colors.volt,       // #D6FF3D
  Poor:      Colors.negative,   // #FF5C6B
};

export function computeWeatherScore(rainPct: number, windMph: number): WeatherScore {
  if (rainPct > 50 || windMph > 25) return 'Poor';
  if (rainPct > 25 || windMph > 18) return 'Fair';
  if (rainPct > 10 || windMph > 12) return 'Good';
  return 'Excellent';
}
...
export function useHourlyWeather(location?: string | null) {
  ...
  const [hourlyWindspeed, setHourlyWindspeed] = useState<number[]>([]);
  ...
  }, [hourlyRain, hourlyTemp, hourlyWindspeed]);
